...
class RobustTextParser:
            ...
            for i, item in enumerate(data):
                try:
                    panel = Panel(
                        ...
                        material=self._normalize_material(item.get('material', 'SGCC')),
    ...
    def parse_cutting_data_tsv(self, raw_data: str) -> Tuple[List[Panel], List[ParseError]]:
        ...
        self.logger.debug(f"Field mapping: {field_map}")
        self.logger.debug(f"Header fields count: {len(header_fields)}")
        self.logger.debug(f"Max field index required: {max(field_map.values()) if field_map else 'None'}")
        ...
        for line_num, line in enumerate(lines[header_line_idx + 1:], header_line_idx + 2):
            ...
            try:
                fields = line.split('\t')
                self.logger.debug(f"Line {line_num}: {len(fields)} fields vs required {max(field_map.values()) + 1 if field_map else 0}")

                if len(fields) < max(field_map.values()) + 1:
                    errors.append(ParseError(
                        line_num, line[:100] + "..." if len(line) > 100 else line,
                        f"Insufficient fields: expected at least {max(field_map.values()) + 1}, got {len(fields)}. Header has {len(header_fields)} fields."
                    ))
                    continue
    ...
    def _parse_boolean(self, value: str) -> bool:
        ...
        false_values = ['false', '', '0', 'no', 'n', 'いいえ', '不可', 'ng']
